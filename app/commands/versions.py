...
def versions() -> None:
    ...
    try:
        ...
        __print_versions_table(versions_info)
    finally:
        client.close()


def __print_versions_table(versions_data: dict) -> None:
    """Print versions in a formatted table"""
    click.echo("Application Versions")
    click.echo("=" * 30)

    # Add CLI version and SHA first
    cli_version = __get_cli_version()
    cli_sha = __get_cli_sha()
