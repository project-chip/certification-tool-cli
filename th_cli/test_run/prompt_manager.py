...
from th_cli.colorize import colorize_log_error, colorize_log_key_value, italic
...
async def handle_prompt(socket: WebSocketClientProtocol, request: PromptRequest) -> None:
    ...
    if isinstance(request, OptionsSelectPromptRequest):
        ...
        click.echo(colorize_log_error(f"Unsupported prompt request: {request.__class__.__name__}"))
    click.echo("=======================================")
...
async def __handle_options_prompt(socket: WebSocketClientProtocol, prompt: OptionsSelectPromptRequest) -> None:
    try:
        ...
        click.echo(colorize_log_error("Prompt timed out"), err=True)
        pass
...
async def __prompt_user_for_option(prompt: OptionsSelectPromptRequest) -> int:
    ...
    for key in prompt.options.keys():
        ...
        click.echo(f"  {colorize_log_key_value(str(id), key)}")
    ...
    click.echo(colorize_log_error(f"Invalid input {input}"), err=True)
